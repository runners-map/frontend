...
export async function generateMetadata({
  ...
  const chatRoom = chats.find(
    (chat: ChatRoom) => chat.chatRoomId === parseInt(id)
  );
      ...
      <main className="flex-1 p-6 bg-white shadow-lg rounded-md mx-4 mt-4">
        <section className="mb-6">
          ...
          <p className="text-lg flex items-center mb-2">
            ...
            출발시간:{" "}
            <span className="font-bold">
              {chatRoom.startTime.split("T")[0]}{" "}
              {chatRoom.startTime.split("T")[1]}
            </span>
          </p>
          <p className="text-lg flex items-center mb-2">
            <FaMapMarkerAlt className="mr-2 text-green-500" />
            출발위치:{" "}
            <span className="font-bold">{chatRoom.startPosition}</span>
          </p>
          <p className="text-lg flex items-center mb-2">
          ...
          <p className="text-lg flex items-center mb-2">
            ...
            <span className="font-bold">
              {chatRoom.paceSec
                ? `${chatRoom.paceMin}분 ${chatRoom.paceSec}초`
                : `${chatRoom.paceMin}분`}
            </span>
          </p>
